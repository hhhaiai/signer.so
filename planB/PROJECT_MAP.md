...
| Native 完整算法 | 独立 C++ 实现 IV、动态 field 0、custom-state field 4、动态 payload、AES-CBC、HMAC 和结果拼装 | frozen Pixel、API23+/API18 `0x07`、176-byte 变体、9-correction/192-byte 与 17-correction/208-byte 动态扩容，共十六组完整原 SO oracle 均逐字节一致。 |
| Java recovered backend | `NativeLibHelper -> RecoveredNativeBackend -> recovered-primitives` | 不加载原 SO/Unidbg；冻结 Pixel 完整 JSON 严格一致。 |
| Native codeword 边界 | 初始化/cmdline/路径/system SDK/certificate/trampoline/timing 与原 SO correction 注入 | 已闭合当前已观察事件语义和顺序；`0x3c` 比较 `ro.build.version.sdk` 与 native Android API；field 0 容量按 `8→16→24→32...` 分块扩容，17 corrections 的 208-byte 原 SO oracle 已闭合。 |
...
最新审计实际通过了 Maven 全量 **72 tests**（0 failure/error/skip）与打包、API descriptor、one-click 两次确定性、
...
| `LegacyAndroidApiNativeIntegrationTest` | API 18 的 SharedPreferences、RSA-wrapped key、Cipher、SecretKeySpec、HmacSHA256；导入既有 PKCS#8/X.509 RSA pair 与 wrapped secret；正确/错误 Java HMAC 的原 SO/recovered `0x07` parity。 |
...
| `test-full.sh` | 首选全量 Maven 回归；逐测试类隔离，只对 macOS Unidbg/JVM native crash 有界重试，最后打包并汇总测试数。 | 否 |
| `test-generate-signer-retry.sh` | 模拟首次退出 134，验证一键 signer 只对已知 native crash 重试一次，普通失败不重试。 | 否 |
...
5. 当前已恢复并在 `native-reimplementation/` 独立编译动态结果算法，已验证 176-byte
   Pixel/变体、API23+/API18 `0x07`、192-byte 九 correction 与 208-byte 十七 correction 结果，并通过十六组原 SO oracle；Java
