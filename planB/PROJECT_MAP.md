...
| Native 加密边界 | 默认关闭的 raw-vector/VM trace 提取了 15 组 round key；标准 AES-256 key schedule 与 JCE 复算均匹配 | 首个 16-byte body block 已证明是固定 key 的 AES-256；当前差异已缩小到 AES 输入的 4 bytes，详见 `SO_REVERSE_STATUS.md`。 |
| Native codeword 边界 | dispatcher trace 和 `0x13063c` 静态/动态恢复 | 已证明缺失事件是 signer 入口 trampoline 检测；配置 `runtime.signerCodeTrampolineDetected=true` 后产生 `0x25` correction，完整 frozen reference 通过。 |

审计时实际通过了 Maven 全量测试与打包、API descriptor、one-click 两次确定性、
...
flowchart LR
  ...
  B --> U["AdjustSignatureRunner + Unidbg"]
...
| `native-reimplementation/` | 独立 C++ 逆向工作台 | 否 | 不加载原 SO/Unidbg，已可编译运行 AES-256 与 `0x13531c` codeword primitive；尚非完整替代。 |
...
- `NativeLibHelper.java`：不直接让 macOS JVM `System.loadLibrary()` Android SO，
  而是将 `nOnResume` / `nSign` 转发到 `AdjustSignatureRunner`。
- `a.java`、`b.java`、`c.java`、`d.java`：从 AAR Java 层恢复的 signer 辅助逻辑。

实现边界：Host JVM 保留 SDK Java 行为，Android ARM64 native 部分始终在 Unidbg
中执行，避免 macOS 直接加载 Android ELF。
...
这是独立于 Java/Unidbg 的 C++ 恢复工作台：

```bash
./native-reimplementation/build-and-test.sh
```

当前会使用系统已有的 `c++` 编译并验证 Pixel 首个 AES-256 block，以及从
`state+0x10` 提取的完整 16-halfword codeword basis 和 `0x2b/0x36/0x25/0x05`
correction vectors。它不会加载原 AAR、JNI、Unidbg 或 `libsigner.so`。目前只是一套
已验证 primitive，不应被描述为完整 signer replacement。
...
4. 当前桌面 signer 与冻结 Pixel 8 reference 已全字节一致；`reference-result.json` 未被修改。
   ...
   desktop 输出伪造通过。
5. 当前已恢复并在 `native-reimplementation/` 独立编译验证首个 body block 的 AES-256
   primitive 与 `0x13531c` codeword encoder，并闭合此前 4-byte pre-AES 差异；但完整
   C/C++/Go SO 替代实现仍未交付。`libsigner.so` 仍由 Unidbg 执行，
   完整逆向状态以
   `SO_REVERSE_STATUS.md` 为准。
