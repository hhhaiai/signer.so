...
class RecoveredNativeBackendIntegrationTest {
    ...
    @Test
    void recoveredSourceBackendDerivesSystemSdkMismatchCorrection() throws Exception {
        File root = findProjectRoot();
        File fixture = new File(root, "device-reference/references/pixel8-api36");
        JSONObject job = JSON.parseObject(
                Files.readString(new File(fixture, "signer-job.json").toPath()), Feature.OrderedField);
    ...
    @Test
    void recoveredSourceBackendAcceptsMatchingSystemSdkIndependentOfApi() throws Exception {
        File root = findProjectRoot();
        File fixture = new File(root, "device-reference/references/pixel8-api36");
        JSONObject job = JSON.parseObject(
                Files.readString(new File(fixture, "signer-job.json").toPath()), Feature.OrderedField);
        ...
        JSONObject device = job.getJSONObject("device");
        device.put("androidApi", 35);
        device.getJSONObject("systemProperties").put("ro.build.version.sdk", "35");
        device.getJSONObject("runtime").put("backend", "recovered");
        ...
        assertEquals(expected.getString("rawSignatureHex"), actual.getString("rawSignatureHex"));
    }

    @Test
    void recoveredSourceBackendUsesUnidbgSdk23FallbackWhenPropertyIsMissing() throws Exception {
        File root = findProjectRoot();
        File fixture = new File(root, "device-reference/references/pixel8-api36");
        JSONObject job = JSON.parseObject(
                Files.readString(new File(fixture, "signer-job.json").toPath()), Feature.OrderedField);
        ...
        device.getJSONObject("systemProperties").remove("ro.build.version.sdk");
        device.getJSONObject("runtime").put("backend", "recovered");
    ...
    @Test
    void recoveredSourceBackendDerivesSparseApi23OriginalSoCorrectionVector() throws Exception {
        File root = findProjectRoot();
    ...
    @Test
    void recoveredSourceBackendDerivesInvalidJavaHmacCorrection07() throws Exception {
        File root = findProjectRoot();
        File fixture = new File(root, "device-reference/references/pixel8-api36");
        JSONObject job = JSON.parseObject(
                Files.readString(new File(fixture, "signer-job.json").toPath()), Feature.OrderedField);
        ...
        java.lang.reflect.Method sign = type.getDeclaredMethod(
                "sign", android.content.Context.class, Object.class, byte[].class, int.class);
        ...
        byte[] actual = (byte[]) sign.invoke(backend, new android.content.Context(profile.getPackageName()),
                params, new byte[32], 36);

        assertEquals("3b273362218b186a73e7349775b93f11caae2e12d78b1701c676ed06c809f444"
                        + "08f6bec00e3171165911abed8c93e1eb50a0aa09eb5b55788c55f80cdbee7f26"
                        + "c8db26b88a2dc6ea03294681738ff23caf8782d1cc9fee8e5264e71a441ff9a56"
                        + "aedcf851f3cd57918a12da004290a8de39d0149424cc33b0a92f7cdd9d35f67c2"
                        + "ffc2edb22e879c91b26ab1246c538aabb2c290829dc739dd64e035feafdaefda12"
                        + "6c69da1301660b2b1ddbfa8822f5", hex(actual));
