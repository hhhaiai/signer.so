#!/usr/bin/env python3
"""Static cross-ABI proof for JNI indexed object reader ARM64 0xa948c."""
...
def main() -> None:
    ...
    arm = body(disassemble(objdump, ARM64_SO, 0xA948C, 0xA9D44),
               0xA948C, 0xA9D44)
    x86 = body(disassemble(objdump, X86_64_SO, 0xA4CD9, 0xA53A9, True),
               0xA4CD9, 0xA53A9)

    require(ARM_EH, r"pc=000a948c\.\.\.000a9d44", "ARM64 FDE")
    require(X86_EH, r"pc=000a4cd9\.\.\.000a53a9", "x86_64 FDE")
    if len(re.findall(r"\bbl\s+0xa948c\b", ARM_FULL)) != 1:
        raise AssertionError("expected one ARM64 caller")
    if len(re.findall(r"\bcallq?\s+0xa4cd9\b", X86_FULL)) != 1:
        raise AssertionError("expected one x86_64 caller")
    require(ARM_FULL, r"\b8e358:.*\bbl\s+0xa948c\b", "ARM64 caller")
    require(X86_FULL, r"\b93ca9:.*\bcallq?\s+0xa4cd9\b", "x86 caller")
    print("cross-ABI FDEs and sole detector-producer caller: PASS")

    method_name = bytes(value ^ 0x47
                        for value in virtual_bytes(ARM64_SO, 0x144FBC, 4))
    signature = bytes(value ^ 0x77
                      for value in virtual_bytes(ARM64_SO, 0x144FC0, 22))
    x86_name = bytes(value ^ 0xC5
                     for value in virtual_bytes(X86_64_SO, 0x13DA5C, 4))
    x86_signature = bytes(value ^ 0x76
                          for value in virtual_bytes(X86_64_SO, 0x13DA60, 22))
    if method_name != b"get\0" or x86_name != b"get\0":
        raise AssertionError(f"unexpected method decode: {method_name!r}/{x86_name!r}")
    expected_signature = b"(I)Ljava/lang/Object;\0"
    ...
    print("cross-ABI get / (I)Ljava/lang/Object; constants: PASS")
    ...
    require_order(arm, [
        ...
        "ldr\tx8, [x8, #272]",
        "blr\tx8",
    ], "ARM64 object/method/index CallObjectMethod arguments")
    print("four JNI vtable methods, int index and three exception stages: PASS")
    ...
    require(arm, r"ldr\s+x8,\s*\[sp,\s*#40\].*ldr\s+x8,\s*\[x8\].*cmp\s+x8,\s*#0x0",
    ...
    start = CPP.index("void runRecoveredJniIndexedObjectMethodReaderA948c(")
    end = CPP.index("enum class RecoveredJniIndexedObjectMethodEventA948c", start)
    ...
    require_order(implementation, [
        ...
        "jniEnvironment, objectClass, \"get\", \"(I)Ljava/lang/Object;\")",
        "operations.consumeException(jniEnvironment)",
        "*status = 18;",
        "*output = operations.callObjectMethod(",
        "jniEnvironment, object, methodId, index)",
        "operations.consumeException(jniEnvironment)",
    ...
    require(CPP, r"bool recoveredJniIndexedObjectMethodReaderA948cRegression\(\)",
            "C++ regression")
    require(CPP,
            r"if \(!recoveredJniIndexedObjectMethodReaderA948cRegression\(\)\)",
            "top-level guard")
    require(GENERATOR,
            r"0x0A948C:\s*\(\"JNI indexed object-method reader\",\s*\"recovered\"",
            "coverage entry")
