...
case "$PLATFORM" in
  Darwin)
    DEFAULT_LIBRARY="$ROOT/build/libsigner_compat.dylib"
    ;;
  Linux)
    DEFAULT_LIBRARY="$ROOT/build/libsigner_compat.so"
    ;;
  *)
...
if grep -Eiq 'FakeSignerBackend[^[:space:]]*::|fake_signer_backend|recovered_primitives|kRecovered' \
    "$ALL_SYMBOLS_FILE"; then
  echo "production library contains fake/recovered implementation symbols" >&2
  grep -Ei 'FakeSignerBackend[^[:space:]]*::|fake_signer_backend|recovered_primitives|kRecovered' \
...
echo "+ $CXX_BIN -std=c++17 -Wall -Wextra -Werror -fsyntax-only $ROOT/recovered_primitives.cpp"
"$CXX_BIN" -std=c++17 -Wall -Wextra -Werror \
  -fsyntax-only "$ROOT/recovered_primitives.cpp"
echo "recovered_primitives.cpp compile-only syntax audit: PASS"
